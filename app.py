...
st.set_page_config(page_title="Mon Quartier Shopping — Lubumbashi", page_icon="🛍️", layout="wide")

BASE_DIR = Path(__file__).resolve().parent
DB_NAME = BASE_DIR / "hyperlocal_shop.db"
UPLOAD_DIR = BASE_DIR / "uploads"
UPLOAD_DIR.mkdir(exist_ok=True)
DEFAULT_NEIGHBORHOOD = "Quartier Nord"
...
def show_client(neighborhood):
    show_cart()
    st.subheader("Produits disponibles dans votre quartier")
    c1, c2 = st.columns([2, 1])
    query = c1.text_input("Article recherché", placeholder="robe, sac, sandales...").lower().strip()
    only_open = c2.checkbox("Boutiques ouvertes")
...
init_db()
st.image(str(BASE_DIR / "logo.svg"), width=220)
st.title("Mon Quartier Shopping")

st.caption("Boutiques et produits locaux de Lubumbashi")
with st.sidebar:
    st.header("Navigation")
    menu = st.radio("Choisir un espace", ["Accueil", "Mode Client", "Espace Commerçant"])
    if menu == "Mode Client":
        neighborhood = st.text_input("Votre quartier", value=DEFAULT_NEIGHBORHOOD, placeholder="Golf, Kenya, Katuba...")
        st.caption("Écrivez Tous pour voir toute la ville.")
if menu == "Accueil":
    show_home()
elif menu == "Mode Client":
    show_client(neighborhood)
else:
    show_merchant()
