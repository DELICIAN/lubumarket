...
def init_db():
    with get_connection() as conn:
        ...
        if "expires_at" not in story_columns:
            conn.execute("ALTER TABLE stories ADD COLUMN expires_at TEXT")
        group_columns = [row[1] for row in conn.execute("PRAGMA table_info(group_orders)").fetchall()]
        if "created_at" not in group_columns:
            conn.execute("ALTER TABLE group_orders ADD COLUMN created_at TEXT")
        if "expires_at" not in group_columns:
            conn.execute("ALTER TABLE group_orders ADD COLUMN expires_at TEXT")
        ...
        # Aucun commerce ni article de démonstration n’est créé automatiquement.
        # L’application doit afficher uniquement les données réellement publiées par les vendeurs.
        demo_shop_names = ("Boutique du Coin", "Supérette Centre")
        demo_product_names = ("Robe d'été fleurie", "Sandales en cuir", "Sac à main chic")
        demo_shops = conn.execute("SELECT id FROM shops WHERE name IN (?, ?)", demo_shop_names).fetchall()
        demo_ids = [row["id"] for row in demo_shops]
        if demo_ids:
            placeholders = ",".join("?" for _ in demo_ids)
            conn.execute(f"DELETE FROM group_orders WHERE product_id IN (SELECT id FROM products WHERE shop_id IN ({placeholders}))", demo_ids)
            conn.execute(f"DELETE FROM stories WHERE shop_id IN ({placeholders})", demo_ids)
            conn.execute(f"DELETE FROM products WHERE shop_id IN ({placeholders})", demo_ids)
            conn.execute(f"DELETE FROM shops WHERE id IN ({placeholders})", demo_ids)
        conn.execute("DELETE FROM products WHERE name IN (?, ?, ?)", demo_product_names)
...
def load_products():
    with get_connection() as conn:
        ...
        s.latitude, s.longitude FROM products p JOIN shops s ON p.shop_id=s.id
        WHERE COALESCE(p.media_type, 'photo') = 'photo' ORDER BY p.id DESC""").fetchall()
...
def show_client():
    ...
    st.caption("Recherchez un article en haut de l’écran. Le quartier est facultatif : écrivez Tous pour afficher les articles réellement publiés.")
    ...
    products = [p for p in all_products if (nearby_ids is None or p["id"] in nearby_ids) and neighborhood_matches(neighborhood, p["address"], p["shop_name"]) and query in normalize_text(f"{p['name']} {p['description']}")]
    ...
    if not products and neighborhood.strip() and normalize_text(neighborhood) not in {"tous", "toutes"}:
        st.info("Aucun article réel n’est publié dans ce quartier pour le moment. Essayez un autre quartier ou Tous.")
    if products:
        cols = st.columns(3)
...
def show_arrivals():
    ...
    now = dt.datetime.now()
    with get_connection() as conn:
        conn.execute("DELETE FROM stories WHERE expires_at IS NOT NULL AND expires_at <= ?", (now.isoformat(timespec="minutes"),))
        arrivals = conn.execute("""SELECT stories.*, shops.name shop_name, shops.address, shops.whatsapp
        FROM stories JOIN shops ON stories.shop_id=shops.id ORDER BY stories.id DESC LIMIT 50""").fetchall()
    active_arrivals = []
    for arrival in arrivals:
        if arrival["expires_at"]:
            try:
                if dt.datetime.fromisoformat(arrival["expires_at"]) <= now:
                    continue
            except ValueError:
                pass
        active_arrivals.append(arrival)
    ...
    for arrival in active_arrivals:
        with st.container(border=True):
            ...
            st.caption(f"Publié le {arrival['created_at']} · Visible pendant 7 jours")
...
def show_group_orders():
    ...
    st.info("Créez une commande avec un article et votre quartier. Vous serez automatiquement la première participante ; vos voisins pourront ensuite rejoindre la même commande et vous verrez leur nombre.")
    with get_connection() as conn:
        conn.execute("DELETE FROM group_orders WHERE expires_at IS NOT NULL AND expires_at <= ?", (dt.datetime.now().isoformat(timespec="minutes"),))
        products = conn.execute("SELECT p.id, p.name, s.name shop_name FROM products p JOIN shops s ON p.shop_id=s.id WHERE p.stock > 0 ORDER BY p.name").fetchall()
    with st.expander("Créer une nouvelle commande groupée", expanded=not bool(products)):
        if not products:
            st.warning("Aucun article réel n’est encore disponible. Un commerçant doit publier un produit avant de créer une commande groupée.")
        else:
            options = {f"{p['name']} — {p['shop_name']}": p['id'] for p in products}
            with st.form("create_group_order"):
                product_label = st.selectbox("Article à acheter en groupe", list(options))
                neighborhood = st.text_input("Votre quartier", placeholder="Golf, Kenya, Katuba...")
                name = st.text_input("Votre nom")
                phone = st.text_input("Votre téléphone")
                target = st.number_input("Nombre souhaité de participants", min_value=2, max_value=100, value=5, step=1)
                if st.form_submit_button("Commencer la commande groupée"):
                    if all(x.strip() for x in [neighborhood, name, phone]):
                        created = dt.datetime.now()
                        expires = created + dt.timedelta(days=7)
                        with get_connection() as conn:
                            cur = conn.execute("INSERT INTO group_orders (product_id, neighborhood, current_participants, target_participants, created_at, expires_at) VALUES (?, ?, ?, ?, ?, ?)", (options[product_label], neighborhood.strip(), 1, int(target), created.isoformat(timespec="minutes"), expires.isoformat(timespec="minutes")))
                            conn.execute("INSERT INTO group_members (group_order_id, customer_name, phone, created_at) VALUES (?, ?, ?, ?)", (cur.lastrowid, name.strip(), clean_phone(phone), created.isoformat(timespec="minutes")))
                        st.success("Commande créée : vous êtes la première participante. Partagez cette page avec vos voisins.")
                        st.rerun()
                    else:
                        st.error("Indiquez votre quartier, votre nom et votre téléphone.")
    with get_connection() as conn:
    ...
    for group in groups:
        with st.container(border=True):
            ...
            if group["expires_at"]:
                st.caption(f"Commande ouverte jusqu’au {group['expires_at'][:16].replace('T', ' ')}")
            with get_connection() as conn:
                members = conn.execute("SELECT customer_name, created_at FROM group_members WHERE group_order_id=? ORDER BY id", (group["id"],)).fetchall()
            if members:
                st.caption("Participants : " + ", ".join(m["customer_name"] for m in members))
...
def show_merchant():
    ...
    with st.form("add_product"):
        ...
        media = st.file_uploader("Ajouter une photo depuis votre téléphone", type=["jpg", "jpeg", "png", "webp"])
        if st.form_submit_button("Ajouter le produit"):
            if not name.strip() or media is None:
                st.error("Le nom et la photo sont obligatoires.")
            else:
                saved_media = save_uploaded_image(media)
                with get_connection() as conn:
                    conn.execute("INSERT INTO products (shop_id, name, description, price, image_url, stock, media_type) VALUES (?, ?, ?, ?, ?, ?, ?)", (shop["id"], name.strip(), description.strip(), price, saved_media, stock, "photo"))
    ...
    with st.form("story"):
        ...
        if st.form_submit_button("Publier la story") and content.strip():
            with get_connection() as conn:
                created = dt.datetime.now()
                expires = created + dt.timedelta(days=7)
                conn.execute("INSERT INTO stories (shop_id, content, created_at, expires_at) VALUES (?, ?, ?, ?)", (shop["id"], content.strip(), created.isoformat(timespec="minutes"), expires.isoformat(timespec="minutes")))
...
with st.sidebar:
    st.header("Autres espaces")
    secondary_menu = st.selectbox("Ouvrir", ["— Choisir —", "Commandes groupées", "Coursier pour essayage", "Espace Commerçant"])
    st.caption("Le paiement n’est pas intégré : les demandes se confirment avec la boutique.")

# Navigation principale inspirée des applications mobiles : affichée en bas de l’écran.
st.markdown("""<style>
section[data-testid='stMain'] div[data-testid='stRadio'] {
    position: fixed; bottom: 0; left: 0; right: 0; z-index: 999;
    background: white; border-top: 1px solid #dddddd; padding: 8px 10px 4px;
    box-shadow: 0 -2px 8px rgba(0,0,0,.08);
}
section[data-testid='stMain'] .block-container { padding-bottom: 90px; }
</style>""", unsafe_allow_html=True)
main_menu = st.radio("Navigation principale", ["Articles", "Arrivages", "Créateurs & Stylistes"], horizontal=True, key="main_bottom_nav", label_visibility="collapsed")

if secondary_menu == "— Choisir —":
    if main_menu == "Articles":
        show_client()
    elif main_menu == "Arrivages":
        show_arrivals()
    else:
        show_creators()
elif secondary_menu == "Arrivages":
    show_arrivals()
elif secondary_menu == "Commandes groupées":
    show_group_orders()
elif secondary_menu == "Coursier pour essayage":
    show_courier_request()
elif secondary_menu == "Espace Commerçant":
    show_merchant()